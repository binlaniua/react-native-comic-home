  ...
  dispatch_async(_processingQueue, ^{
    ...
    if (pendingBlocks) {
      ...
      if ([_cache hasDataForKey:cacheKey]) {
        ...
        NSRange referRange = [urlStr rangeOfString:@"refer"];
        NSString *refer = nil;
        if (referRange.length != 0){
          refer = [urlStr substringFromIndex: NSMaxRange(referRange) + 1];
          urlStr = [urlStr substringToIndex:NSMaxRange(referRange) - 6];
        }
        ...
        if (refer != nil){
          [request setAllHTTPHeaderFields:@{@"User-Agent": @"Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.95 Safari/537.36", @"Referer": refer}];
        }
